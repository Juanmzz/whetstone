...
6. [DL6] Don't reorganize a dev's personal-doc structure, meaning create, rename or move
...
- code, identifiers, comments, paths, command names, agent-config `.md` → the codebase's
  primary language (usually English, since it's read by the agent);
...
9. [DL9] **No em-dash.** A dash standing in for a colon, a comma or a full stop is the
   house style of generated text, and a reader who has seen enough of it stops reading for
   content. Pick the punctuation that says what the clause is doing: a colon introduces, a
   comma joins, a full stop separates. Applies to every artifact the agent writes, prose and
   commit bodies alike. Append-only records are exempt, because a decision changes by status
   and never by rewrite.

## Changelog

- v4 (2026-08-25, owner decision): added [DL9] after the owner read the docs before making
  the repo public and found 75 em-dashes across the five files a stranger reads first. Not
  machine-checked yet: a check would be trivial to write and would have to exempt
  `decisions.md` and `retro-log.md`, so it waits until something other than style asks for
  it.
