...
export function activeSkills(present?: readonly string[]): readonly string[] {
  ...
  // alphabetical (adr-0041). Anything the payload does not ship was written by
...
export interface WstYamlInput {
  ...
  /** The adapter that runs llm checks. Omitted means `claude`, which is the default. */
  readonly agent?: string;
}

export function renderWstYaml(input: WstYamlInput): string {
  // It hardcoded `claude`, so picking Antigravity got you a draft from
  // antigravity and a config naming claude (adr-0040).
  ...
  const lines = [
    ...
    "version: 0",
    `agent: ${agent.padEnd(18)} # which adapter runs llm checks`,
